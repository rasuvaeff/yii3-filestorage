...
`url()` и `temporaryUrl()` отдают шаги 1 и 2 для инфраструктурного кода.
«В сыром виде» — только наполовину: `url()` игнорирует политику выдачи
полностью, а `temporaryUrl()` передаёт хранилищу `DeliveryOptions` группы и
возвращает null, если хранилище не может их соблюсти. Ни тот, ни другой не
смотрит на `allowDirectPublicUrl` — этот шлюз принадлежит `urlFor()`. Прикладной код и шаблоны не должны ветвиться на «а
...
| Команда | Что делает |
|---|---|
| `filestorage:check` | Показывает проводку, возможности каждого хранилища, тенантный режим и правила каждой группы; падает на небезопасном сочетании выдачи и на тенантном режиме без scoped-резолвера |
| `filestorage:stat` | Количество и размер по группам плюс то, сколько сэкономило разделение объектов (физические цифры не выводятся в тенантном режиме) |
...
| `filestorage:import <dir>` | Загружает дерево каталогов обычным путём записи, пропуская то, что манифест уже записал как импортированное |
...
удаляет байты, нужные другому запросу, и делает это под эксклюзивной арендой блоба с
...
`gc --orphans --apply`. В тенантном режиме последний шаг переезжает: `deduplicate`
...
### Импорт существующего дерева каталогов

`filestorage:import` обходит каталог и прогоняет каждый файл через `add()` — та
же проверка политики, то же определение MIME, та же запись в хранилище и та же
строка метаданных, что и у живой загрузки. Он никогда не вставляет строку и не
пишет объект напрямую — поэтому приложение, включившее дедупликацию, получает
дедуплицированный импорт бесплатно: команда резолвит `StorageInterface`, а это
ровно тот ключ, который такое приложение переопределило.

```bash
./yii filestorage:import /srv/legacy/invoices                          # отчёт
...
|---|---|---|
| `--apply` | выкл | Без неё ничего не импортируется и манифест не трогается |
| `--group` | `defaultGroup` | Одна группа на весь прогон. Разные группы — разные прогоны по разным подкаталогам |
| `--store` | хранилище по умолчанию | В какое физическое хранилище писать |
| `--limit` | 1000 | Файлов за прогон. Ограничивает работу, но не память — список строится и сортируется заранее, то есть пропорционален всему дереву |
| `--manifest` | `build/filestorage-import.jsonl` | Куда записываются завершённые импорты и откуда читаются, чтобы их пропустить |

**Безопасность повторного прогона держится на манифесте.** У `add()` нет
естественного ключа, поэтому без манифеста повторный прогон запишет вторую
строку **и** второй объект на каждый файл. Манифест — JSON Lines, по
`{"source":…,"path":…,"id":…}` на завершённый файл, со сбросом на диск по ходу
дела: прогон, убитый на середине, оставляет записанным то, что успел, а
следующий подхватывает только новое. Храните его, иначе поздний прогон
продублирует всё. Сопоставление идёт по `source` — абсолютному пути, — поэтому
**один манифест обслуживает несколько корней**: две команды выше делят манифест
по умолчанию и не пропускают файлы друг друга, даже когда в обоих деревьях есть
`notes.txt`.

Файл, отвергнутый политикой, показывается, пропускается и **не** записывается в
манифест — поэтому прогон после расширения политики повторит ровно его. При
любых сбоях команда завершается ненулевым кодом.

Символические ссылки не разыменовываются (ссылка в легаси-дереве может указывать
за пределы названного каталога), записи, начинающиеся с точки, пропускаются на
любой глубине, а исходный путь сохраняется в `metadata['importSource']` строки.

### Тенантный режим и подметание orphans

> Термины: **арендатор** — тенант, значение `FileScopeProviderInterface`.
> **Аренда** — исключительно `BlobLease`, эксклюзивный захват блоба сборщиком.
> Это разные механизмы, и слово «аренда» здесь всегда про второй.
...
чему. Сравнение этих двух в тенантном режиме объявляет orphan'ами все объекты остальных
