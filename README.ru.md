...
| `--limit` | 1000 | Файлов за прогон. Дерево больше одного захода — последовательность прогонов |
...
`{"path":…,"id":…}` на завершённый файл, со сбросом на диск по ходу дела: прогон,
убитый на середине, оставляет записанным то, что успел, а следующий подхватывает
только новое. Храните его, иначе поздний прогон продублирует всё.
