...
| Команда | Что делает |
|---|---|
| `filestorage:check` | Показывает проводку, возможности каждого хранилища, режим аренды и правила каждой группы; падает на небезопасном сочетании выдачи и на режиме аренды без scoped-резолвера |
...
`gc --orphans --apply`. При аренде последний шаг переезжает: `deduplicate`
прогоняется по разу на арендатора в окружающей области, а подметание
отказывается работать при привязанном провайдере областей и запускается один раз
с непривязанным (см. ниже).

### Аренда и подметание orphans

`filestorage:check` сообщает, в каком режиме вы находитесь, и **падает**, когда
привязан `FileScopeProviderInterface`, а `ScopedFileResolverInterface` не
привязан никем: у подписанной загрузки тогда нет способа разрешить файл в
пределах области, а разрешение по одному id читает любой файл, чей id утёк.
Поставьте бэкенд репозитория с резолвером (`-db` его даёт) либо отвяжите
провайдер областей, если установка не multi-tenant. Обратное — резолвер без
провайдера областей — обычный одно-областной случай, и это нормально.
