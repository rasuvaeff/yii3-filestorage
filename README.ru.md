...
| `filestorage:stat` | Количество и размер по группам плюс то, сколько сэкономило разделение объектов |
...
`filestorage:stat` читает через тот же ограниченный репозиторий, поэтому при
привязанном провайдере областей его цифры — включая «Distinct objects» и
экономию от разделения — описывают только текущего арендатора. Это более узкий
отчёт, а не неверный, но это не итог по установке.
